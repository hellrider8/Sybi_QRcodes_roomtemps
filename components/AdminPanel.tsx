...
import { Save, Trash2, X, Shield, RefreshCw, Wifi, CheckCircle2, AlertCircle, Copy, Search, Download } from 'lucide-react';
  ...
  const handleDiscover = async () => {
    ...
    try {
      ...
      if (result.rooms && result.rooms.length > 0) {
        ...
        alert(`${result.rooms.length} Räume gefunden (Gruppen wurden ignoriert).`);
      } else { alert("Keine passenden Räume gefunden."); }
          ...
          <div className="max-w-md mx-auto space-y-6">
            <div className="bg-white p-6 border rounded-xl shadow-sm space-y-4">
              ...
              <div className="flex p-1 bg-slate-50 border rounded-lg">
                <button onClick={() => setConfig({...config, apiMode: 'local'})} className={`flex-1 py-2 rounded text-[10px] font-bold transition-all ${config.apiMode === 'local' ? 'bg-white text-[#00828c] shadow-sm' : 'text-slate-500'}`}>LOKAL (LAN)</button>
                <button onClick={() => setConfig({...config, apiMode: 'cloud'})} className={`flex-1 py-2 rounded text-[10px] font-bold transition-all ${config.apiMode === 'cloud' ? 'bg-white text-[#00828c] shadow-sm' : 'text-slate-500'}`}>CLOUD (WAN)</button>
              </div>
              ...
              {config.apiMode === 'cloud' && (
                <div className="bg-slate-50 p-4 rounded-lg space-y-3">
                  ...
                  <div className="flex p-1 bg-white border rounded-md">
                    <button onClick={() => setConfig({...config, cloudProvider: 'gekko'})} className={`flex-1 py-1.5 rounded text-[9px] font-bold transition-all ${config.cloudProvider === 'gekko' ? 'bg-[#00828c] text-white shadow-sm' : 'text-slate-500'}`}>myGEKKO</button>
                    <button onClick={() => setConfig({...config, cloudProvider: 'tekko'})} className={`flex-1 py-1.5 rounded text-[9px] font-bold transition-all ${config.cloudProvider === 'tekko' ? 'bg-[#00828c] text-white shadow-sm' : 'text-slate-500'}`}>TEKKO</button>
                  </div>
                </div>
              )}

              <input type="text" placeholder={config.apiMode === 'local' ? "IP-Adresse (z.B. 192.168.1.5)" : "Cloud-ID (z.B. 99XY-8NTZ...)"} className="admin-input" value={config.apiMode === 'local' ? config.ip : config.gekkoId} onChange={e => setConfig(config.apiMode === 'local' ? {...config, ip: e.target.value} : {...config, gekkoId: e.target.value})} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
               {config.rooms.map(r => (
                 <div key={r.id} className="bg-white p-6 border rounded-2xl flex flex-col items-center shadow-sm hover:shadow-md transition-shadow">
                   ...
                   <button onClick={() => copyToClipboard(getQrUrl(r.id))} className="mt-4 text-[9px] text-slate-400 uppercase font-bold flex items-center gap-1 hover:text-[#00828c]"><Copy size={10}/> Direktlink kopieren</button>
                 </div>
               ))}
            </div>
          </div>
        ...
        {activeTab === 'hosting' && (
          <div className="max-w-md mx-auto space-y-6">
            <div className="bg-white p-6 border rounded-2xl shadow-sm space-y-6">
               <h3 className="font-bold text-xs uppercase text-[#00828c] border-b pb-2">Regeln & Sicherheit</h3>
